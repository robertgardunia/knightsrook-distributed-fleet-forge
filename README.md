...
The chaos agent does not target itself or homebase (homebase is the fleet socket server — stopping it disconnects the dashboard). It can target the Fireman container — the recovery system going dark while failures are unfolding is a valid story. Requires `ANTHROPIC_API_KEY` in the host environment.

**Auto loop** is off by default. The `○ Auto` toggle in the dashboard header enables the automatic chaos loop (random interval between `MIN_INTERVAL_MS` and `MAX_INTERVAL_MS`); disabling it pauses the loop without stopping the container. Manual trigger (`⚡ Chaos` button) fires a single forced step regardless of auto state — useful for commissioning tests without enabling continuous chaos.

Every fault the chaos agent injects carries `source: 'chaos'` and an `at` timestamp on the `chaos:action` event. This lets the server and dashboard distinguish chaos-injected faults from organic failures even when both occur in the same time window — the prefix disambiguates simultaneous events.
...
- **`@knightsrook/codes/catcher`** — `CodeCaptureService`: active-identity slot. `capture(payload)` accepts a plain code string or JSON `AccountObject` (`{ code, eventId, eventName }`); emits `user:identified` with the full `AccountObject`. `current()` returns `AccountObject | null`. No adapter code — container wires its own input sources.
- **`@knightsrook/codes/batcher`** — `CodeBatcher`: `getAccounts(count)` fetches `AccountObject[]` from Moodle (`eventId`/`eventName` stamped by Moodle); falls back to offline generation. `claim(code, data)` fire-and-forgets `{ code, data }` to Moodle — queues locally on failure. `sync()` drains the failed-claim queue on reconnect. `MoodleConfig` requires `eventCode` + `eventName` (env: `EVENT_CODE` / `EVENT_NAME`).

Moodle endpoint: `POST https://<host>/local/tsn_extauth/api.php?action=generate&...` for accounts; `POST ?action=claim_code` with JSON `{ code, data }` for claims. Auth: `X-Api-Key` header. Extraction to its own repo mirrors `@knightsrook/xapi`.
