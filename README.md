...
| Lower-left | Stats/Screen | Two tabs: Stats (uptime, CPU, RAM, disk, net, ping, location, ID) and Screen (role-aware: game kiosks show HexGL screenshot, info kiosks cycle through simulator slides as a slideshow, controllers/homebase show system status) |
...
> **Windows note:** Vite proxy targets use `127.0.0.1` instead of `localhost` to avoid the IPv6/IPv4 mismatch in Node 18+ on Windows. The socket.io proxy entry uses `changeOrigin: true` so the `Host` header matches the backend and the socket.io handshake succeeds.
